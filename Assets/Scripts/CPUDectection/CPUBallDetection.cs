...
public class CPUBallDetection : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (!collision.GetComponent<BallManager>().hasOwner)
            {
    ...
    private void Update()
    {
        ...
        {
            if (objectsInArea[i].GetComponent<BallManager>().hasOwner)
            {
        ...
        {
            cpuBrain.closestFreeBall = closestBall.gameObject; // Set the ClosestBall
        }
        else
        {
            closestBall = null;
