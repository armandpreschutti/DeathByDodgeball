...
public class PawnCrosshairHandler : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        playerName = GetComponent<PawnManager>().playerName;
    }
    void Update()
    {
        targetObject = playerStateMachine.CurrentTarget;
        if (targetObject != null)
        {
            // Set the crosshair's position on the screen
            crosshair.transform.position = targetObject.transform.position;
